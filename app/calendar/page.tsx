...
const post={id:Date.now(),day:newPost.date.getDate(),title:newPost.title,emoji:newPost.emoji,rating:newPost.rating,content:newPost.content,image:newPost.imageUrl,hashtags:newPost.hashtags};
setPosts([...posts,post]);
};
return(
...
{selectedDay&&(<div className="w-1/3"><div className="sticky top-4 rounded-2xl bg-white p-6 shadow-lg"><div className="mb-4 flex items-center justify-between"><h3 className="text-xl font-bold text-emerald-900">{month+1}월 {selectedDay}일</h3><button onClick={()=>setSelectedDay(null)} className="text-gray-400 hover:text-gray-600 text-2xl">×</button></div>{selectedPosts.length>0?(<div className="space-y-6 max-h-[70vh] overflow-y-auto">{selectedPosts.map(post=>(<div key={post.id} className="pb-6 border-b last:border-0"><div className="mb-3"><div className="flex items-center gap-2 mb-2"><span className="text-3xl">{post.emoji}</span><div><h4 className="font-bold">{post.title}</h4><div className="flex">{Array.from({length:post.rating}).map((_,i)=>(<span key={i}>🍀</span>))}</div></div></div></div>{post.image&&(<img src={post.image} alt={post.title} className="w-full h-48 object-cover rounded-lg mb-3"/>)}<p className="text-sm text-gray-700 mb-3 leading-relaxed">{post.content}</p><div className="flex flex-wrap gap-2">{post.hashtags.map((tag,i)=>(<span key={i} className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full">{tag}</span>))}</div></div>))}</div>):(<div className="text-center py-12 text-gray-400"><p className="text-4xl mb-2">🍀</p><p>이 날의 소확행이 없습니다</p><button onClick={()=>setIsModalOpen(true)} className="mt-4 text-emerald-600 hover:text-emerald-700 font-semibold">+ 추가하기</button></div>)}</div></div>)}</div>
