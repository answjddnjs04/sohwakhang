...
id:number;day:number;title:string;emoji:string;rating:number;content:string;image?:string;
}
export default function CalendarPage(){
const [currentDate,setCurrentDate]=useState(new Date());
const [selectedDay,setSelectedDay]=useState<number|null>(null);
const [isModalOpen,setIsModalOpen]=useState(false);
const [posts,setPosts]=useState<HappinessPost[]>([
{id:1,day:15,title:'카페에서 책 읽기',emoji:'📚',rating:4,content:'오늘 동네 카페에서 따뜻한 라떼와 함께 책을 읽었어요. 창밖으로 보이는 가을 풍경이 너무 좋았습니다.',image:'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=800'},
{id:2,day:18,title:'반려동물과 산책',emoji:'🐕',rating:5,content:'강아지와 함께 공원을 산책했어요! 날씨도 좋고 강아지가 너무 좋아해서 행복했습니다',image:'https://images.unsplash.com/photo-1548199973-03cce0bbc87b?w=800'},
{id:3,day:22,title:'맛있는 브런치',emoji:'🥐',rating:4,content:'주말 아침, 좋아하는 브런치 카페에서 여유로운 시간을 보냈어요. 크루아상이 정말 맛있었습니다!',image:'https://images.unsplash.com/photo-1464305795204-6f5bbfc7fb81?w=800'},
]);
...
const getPostsForDay=(day:number)=>{return posts.filter(p=>p.day===day&&month===9);};
const selectedPosts=selectedDay?getPostsForDay(selectedDay):[];
const handleCreatePost=(newPost:any)=>{
const post={id:Date.now(),day:newPost.date.getDate(),title:newPost.title,emoji:newPost.emoji,rating:newPost.rating,content:newPost.content,image:newPost.imageUrl};
setPosts([...posts,post]);
};
return(
...
<div className="grid grid-cols-7 gap-2">{days.map((day,index)=>{if(day===null)return <div key={'empty-'+index} className="aspect-square"/>;const dayPosts=getPostsForDay(day);const isSelected=day===selectedDay;return(<div key={day} onClick={()=>setSelectedDay(day)} className={'aspect-square rounded-lg border-2 p-2 transition hover:shadow-md cursor-pointer '+(isSelected?'border-emerald-600 bg-emerald-100 shadow-lg':'border-gray-200 bg-white hover:border-emerald-300')}><div className={'text-sm font-semibold '+(isSelected?'text-emerald-800':'text-gray-700')}>{day}</div><div className="mt-1 space-y-1">{dayPosts.map(post=>(<div key={post.id} className="rounded bg-emerald-100 px-1 py-0.5 text-xs"><span>{post.emoji}</span></div>))}</div></div>);})}</div></div></div>
{selectedDay&&(<div className="w-1/3"><div className="sticky top-4 rounded-2xl bg-white p-6 shadow-lg"><div className="mb-4 flex items-center justify-between"><h3 className="text-xl font-bold text-emerald-900">{month+1}월 {selectedDay}일</h3><button onClick={()=>setSelectedDay(null)} className="text-gray-400 hover:text-gray-600 text-2xl">×</button></div>{selectedPosts.length>0?(<div className="space-y-6 max-h-[70vh] overflow-y-auto">{selectedPosts.map(post=>(<div key={post.id} className="pb-6 border-b last:border-0"><div className="mb-3"><div className="flex items-center gap-2 mb-2"><span className="text-3xl">{post.emoji}</span><div><h4 className="font-bold">{post.title}</h4><div className="flex">{Array.from({length:post.rating}).map((_,i)=>(<span key={i}>🍀</span>))}</div></div></div></div>{post.image&&(<img src={post.image} alt={post.title} className="w-full h-48 object-cover rounded-lg mb-3"/>)}<p className="text-sm text-gray-700 leading-relaxed">{post.content}</p></div>))}</div>):(<div className="text-center py-12 text-gray-400"><p className="text-4xl mb-2">🍀</p><p>이 날의 소확행이 없습니다</p><button onClick={()=>setIsModalOpen(true)} className="mt-4 text-emerald-600 hover:text-emerald-700 font-semibold">+ 추가하기</button></div>)}</div></div>)}</div>
<div className="mt-8 text-center"><button onClick={()=>setIsModalOpen(true)} className="rounded-full bg-emerald-600 px-8 py-3 text-lg font-semibold text-white transition hover:bg-emerald-700 shadow-lg">+ 새로운 소확행 추가</button></div></main>
<CreatePostModal isOpen={isModalOpen} onClose={()=>setIsModalOpen(false)} onSubmit={handleCreatePost} selectedDate={selectedDay?new Date(year,month,selectedDay):new Date()}/>
</div>);
}
